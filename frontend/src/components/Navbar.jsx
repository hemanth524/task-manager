...
const Navbar = () => {
  ...
  return (
    <>
    <div>
      <header className='flex justify-between sticky top-0 p-4 bg-blue-200 shadow-sm items-center'>
        <h2 className='cursor-pointer uppercase text-3xl'>
          <Link to="/"> Task Manager </Link>
        </h2>
        <ul className='hidden md:flex gap-4 uppercase font-medium'>
          {authState.isLoggedIn ? (
            <>
              <li className="bg-orange-500 text-white hover:bg-blue-600 font-medium rounded-md">
                <Link to='/tasks/add' className='block w-full h-full px-4 py-2'> <i className="fa-solid fa-plus"></i> Add task </Link>
              </li>
              <li className='py-2 px-3 cursor-pointer text-zxl text-white bg-orange-500 transition rounded-md ' onClick={handleLogoutClick}>Logout</li>
            </>
          ) : (
            <li className='py-2 px-3 cursor-pointer text-zxl text-white bg-orange-500 transition rounded-md'><Link to="/login">Login</Link></li>
          )}
        </ul>
